...
public class OrderRoutes extends RouteBuilder {
    ...
    public void configure() {
        ...
        from("direct:addOrder")
                ...
                .unmarshal().json(JsonLibrary.Jackson, order.class) // Giải mã JSON thành đối tượng Order
                .process(exchange -> {
                ...
                .setHeader(Exchange.CONTENT_TYPE, constant("application/json")) // Thiết lập Content-Type
                .setBody(constant("{\"message\": \"Order added successfully\"}"));
        from("servlet://cors-filter")
                .process(exchange -> {
                    exchange.getMessage().setHeader("Access-Control-Allow-Origin", "*");
                    exchange.getMessage().setHeader("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE, OPTIONS");
                    exchange.getMessage().setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization");
                })
                .to("log:cors-filter");
